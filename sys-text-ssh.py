...
Многопоточно подключается по ssh ко всем заданным устройствам и выполняет на всех заданных устройствах конфигурационную huawei-команду из файла.
'''
...
def send_and_parse_conf_parallel(devices, account, config_file, disable_echo, limit=16):
    '''
    Загружает huawei-команду из файла.
    Многопоточно подключается по ssh ко всем заданным устройствам и выполняет на всех заданных устройствах загруженную huawei-команду.
    '''
    config = cu.load_config_from_text(config_file)
    vendor = 'huawei'
    with ThreadPoolExecutor(max_workers=limit) as executor:
        result = executor.map(cu.send_config_commands_ip_user, devices, repeat(account), repeat(config), repeat(vendor))
...
def create_parser():
    ...
    '''
    desprg = 'Скрипт подключается по ssh к устройству (устройствам) и выполняет конфигурационные huawei-команды из файла.'
    parser = argparse.ArgumentParser(prog = 'sys-text-ssh',
                                        description = desprg,
                                        add_help = False,
    ...
    pr_group.add_argument('-c',
                          dest='config',
                          required=True,
                          help='файл с конфигурационными huawei-командами (без заголовка "sys" и без окончания "quit")')
